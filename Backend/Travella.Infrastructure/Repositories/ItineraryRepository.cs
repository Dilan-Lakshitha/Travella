...
VALUES (@ItineraryId, @StaffId);";
        ...
        public async Task<List<ItineraryListItemDto>> GetGuestItinerariesAsync(int guestId)
        {
            const string sql = """
                SELECT
                    ...
                    i.start_date AS StartDate,
                    i.end_date AS EndDate,
                    LOWER(TRIM(i.status)) AS RawStatus,
        ...
        public async Task<List<ItineraryListItemDto>> GetSubmittedItinerariesAsync(int companyId)
        {
            const string sql = """
                ...
                WHERE i.company_id = @CompanyId
                  AND LOWER(TRIM(i.status)) IN ('submitted', 'under_review', 'returned_for_correction', 'resubmitted')
        ...
        {
            if (!StaffItineraryTabHelper.TryResolveStatuses(tab, out var statuses))
            {
                throw new ArgumentException($"Unknown staff tab '{tab}'.", nameof(tab));
            }
            ...
            const string sql = """
                SELECT
                    ...
                    i.updated_at::date AS SubmittedDate,
                    i.company_id AS CompanyId,
                    COALESCE(p.total_amount, i.total_price, 0) AS TotalPrice,
                    (
                        SELECT m.message
                        FROM tbl_itinerary_messages m
                        WHERE m.itinerary_id = i.id
                ...
                WHERE i.company_id = @CompanyId
                  AND LOWER(TRIM(i.status)) = ANY(@Statuses)
                ORDER BY i.updated_at DESC, i.created_at DESC
            """;

            using var connection = await OpenStandaloneConnectionAsync();
            var rows = await connection.QueryAsync<ItineraryListItemDto>(
                sql,
                new { CompanyId = companyId, Statuses = statuses });
            return rows.ToList();
        }

        public Task<List<ItineraryListItemDto>> GetCompanyItinerariesAsync(int companyId)
            => GetCompanyItinerariesAllAsync(companyId);

        public async Task<List<ItineraryListItemDto>> GetCompanyItinerariesAllAsync(int companyId)
        {
            const string sql = """
                SELECT
                    i.id,
        ...
        public async Task<Dictionary<string, int>> GetCompanyItineraryStatusCountsAsync(int companyId)
        {
            const string sql = """
            ...
            using var connection = await OpenStandaloneConnectionAsync();
            var rows = await connection.QueryAsync<(string Status, int Count)>(sql, new { CompanyId = companyId });
            return rows.ToDictionary(r => r.Status, r => r.Count, StringComparer.OrdinalIgnoreCase);
        }
        ...
        public async Task<List<ItineraryMessageDto>> GetItineraryMessagesAsync(int itineraryId, bool includeInternalNotes)
        {
            const string sql = """
                SELECT
                    m.id,
                    m.itinerary_id AS ItineraryId,
                    m.sender_id AS SenderId,
                    COALESCE(u.name, 'User') AS SenderName,
                ...
                WHERE m.itinerary_id = @ItineraryId
                  AND (@IncludeInternalNotes OR m.type <> 'INTERNAL_NOTE')
                ORDER BY m.created_at ASC, m.id ASC;
            """;

            using var connection = await OpenStandaloneConnectionAsync();
            var rows = await connection.QueryAsync<ItineraryMessageDto>(
                sql,
                new { ItineraryId = itineraryId, IncludeInternalNotes = includeInternalNotes });
            return rows.ToList();
        }

        public async Task SetAssignedReviewerAsync(int itineraryId, int reviewerUserId)
        {
            const string sql = """
                UPDATE tbl_itineraries
                SET assigned_reviewer_id = @ReviewerUserId
                WHERE id = @ItineraryId;
            """;
            ...
            {
                var (conn, tran) = RequireActiveTransaction();
                await conn.ExecuteAsync(sql, new { ItineraryId = itineraryId, ReviewerUserId = reviewerUserId }, tran);
                return;
            }

            using var connection = await OpenStandaloneConnectionAsync();
            await connection.ExecuteAsync(sql, new { ItineraryId = itineraryId, ReviewerUserId = reviewerUserId });
        }

        public async Task<bool> TryAssignReviewerIfUnsetAsync(int itineraryId, int reviewerUserId)
        {
            const string sql = """
                UPDATE tbl_itineraries
                SET assigned_reviewer_id = @ReviewerUserId
            ...
            if (_unitOfWork.HasActiveTransaction)
            {
                ...
                var affected = await conn.ExecuteAsync(
                    sql,
                    new { ItineraryId = itineraryId, ReviewerUserId = reviewerUserId },
                    tran);
                return affected > 0;
            }
        ...
        public async Task<int?> GetAssignedReviewerIdAsync(int itineraryId)
        {
            const string sql = """
