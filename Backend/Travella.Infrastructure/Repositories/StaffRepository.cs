...
namespace Travella.Infrastructure.Repositories
{
    public class StaffRepository : IStaffRepository
    {
        ...
        {
            const string sql = @"
SELECT id, name, company_id AS CompanyId, type AS Role, phone AS Phone, experience AS Experience, availability AS Availability, language AS Language, email AS Email
FROM tbl_staff
WHERE id = @Id;";
        ...
        {
            const string sql = @"
SELECT s.id, s.name, s.company_id AS CompanyId, s.type AS Role, s.phone AS Phone, s.experience AS Experience, s.availability AS Availability, s.language AS Language, s.email AS Email
FROM tbl_staff s
WHERE NOT EXISTS (
    SELECT 1
...
ORDER BY id DESC;";
        ...
        {
            const string sql = @"
INSERT INTO tbl_staff (name, company_id, type, phone, experience, availability, language, email)
VALUES (@Name, @CompanyId, @Role, @Phone, @Experience, @Availability, @Language, @Email)
RETURNING id;";
            ...
            return await connection.ExecuteScalarAsync<int>(
                ...
                new
                {
                    ...
                    Language = staffResource.Language,
                    Email = staffResource.Email
                },
...
WHERE staff_id = @StaffId
  ...
  AND UPPER(status) = 'BOOKED';";
            ...
            var connection = _unitOfWork.HasActiveTransaction
                ? _unitOfWork.Connection
                : GetConnection(useTransaction: false);
            var transaction = _unitOfWork.HasActiveTransaction
                ? _unitOfWork.CurrentTransaction
                : null;
